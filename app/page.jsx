...
export default function Home() {
  ...
  const chooseFile = (nextFile) => {
    if (nextFile && (["audio/mpeg", "audio/wav", "audio/mp4", "audio/webm", "audio/x-m4a"].some((type) => nextFile.type === type) || nextFile.type.startsWith("video/") || nextFile?.name.match(/\.(mp3|wav|m4a|webm|mp4|mov|avi|mkv|mpeg|mpg)$/i))) {
      setFile(nextFile); setError("");
    } else setError("Choose an MP3, WAV, M4A, WEBM, MP4, MOV, AVI, MKV, MPEG, or MPG file.");
  };
  ...
  return <main className={`app-shell ${dark ? "dark-mode" : ""}`}>
    ...
    {showSettings && <div className="settings-panel panel-shadow sans"><div className="settings-title"><KeyRound size={17} /><strong>AI provider</strong><button className="close-settings" aria-label="Close settings" onClick={() => setShowSettings(false)}><X size={15} /></button></div><p>Choose a provider and model. Your key stays in this browser and is sent only with processing requests.</p><select className="provider-select" value={providerDraft} onChange={(event) => { const nextProvider = event.target.value; const nextModel = localStorage.getItem(`morrow-${nextProvider}-model`) || (nextProvider === "gemini" ? "gemini-3.6-flash" : nextProvider === "claude" ? "claude-sonnet-4-20250514" : "gpt-4o-mini"); setProviderDraft(nextProvider); setKeyDraft(localStorage.getItem(`morrow-${nextProvider}-key`) || ""); setModelDraft(nextModel); }}><option value="openai">OpenAI · Whisper + GPT</option><option value="gemini">Google Gemini · audio + analysis</option><option value="claude">Anthropic Claude · text analysis</option><option value="cursor">Cursor · custom endpoint</option></select><input value={modelDraft} onChange={(event) => setModelDraft(event.target.value)} placeholder={providerDraft === "gemini" ? "gemini-3.6-flash" : providerDraft === "claude" ? "claude-sonnet-4-20250514" : providerDraft === "cursor" ? "Cursor model name" : "gpt-4o-mini"} autoComplete="off" /><input type="password" value={keyDraft} onChange={(event) => setKeyDraft(event.target.value)} placeholder={providerDraft === "gemini" ? "Google API key" : providerDraft === "claude" ? "Anthropic API key" : providerDraft === "cursor" ? "Cursor API key" : "OpenAI API key"} autoComplete="off" /><div className="settings-actions"><button onClick={() => { const nextModel = modelDraft.trim(); localStorage.setItem(`morrow-${providerDraft}-key`, keyDraft.trim()); localStorage.setItem(`morrow-${providerDraft}-model`, nextModel); localStorage.setItem("morrow-ai-provider", providerDraft); setProvider(providerDraft); setApiKey(keyDraft.trim()); setModel(nextModel); setShowSettings(false); setError(""); }}>Save key and model</button><button className="remove-key" onClick={() => { localStorage.removeItem(`morrow-${providerDraft}-key`); setKeyDraft(""); setApiKey(""); }}>Remove key</button></div></div>}
    ...
    <section className="workspace">
      <div className="upload-panel panel-shadow"><div className={`dropzone ${dragging ? "dragging" : ""}`} onDragOver={(event) => { event.preventDefault(); setDragging(true); }} onDragLeave={() => setDragging(false)} onDrop={(event) => { event.preventDefault(); setDragging(false); chooseFile(event.dataTransfer.files[0]); }} onClick={() => inputRef.current?.click()}>
        <input ref={inputRef} type="file" hidden accept=".mp3,.wav,.m4a,.webm,.mp4,.mov,.avi,.mkv,.mpeg,.mpg,audio/*,video/*" onChange={(event) => chooseFile(event.target.files[0])} />
        {file ? <><FileAudio size={29} strokeWidth={1.5} /><strong>{file.name}</strong><small>{(file.size / 1024 / 1024).toFixed(2)} MB · Ready to process</small><button className="clear-file sans" onClick={(event) => { event.stopPropagation(); setFile(null); }}><X size={14} /> Remove</button></> : <><div className="upload-icon"><Upload size={23} /></div><strong>Drop your recording here</strong><small>or click to browse · MP3, WAV, M4A, WEBM, MP4, MOV</small></>}
      </div>{audioUrl && (file?.type.startsWith("video/") || file?.name.match(/\.(mp4|mov|avi|mkv|mpeg|mpg)$/i) ? <video className="audio-player" controls src={audioUrl} /> : <audio className="audio-player" controls src={audioUrl} />)}<div className="upload-controls sans"><label>Audio language <div className="select-wrap"><select value={mode} onChange={(event) => setMode(event.target.value)}><option value="en-to-en">English audio {"to"} English transcript</option><option value="hi-to-en">Hindi audio {"to"} English transcript</option></select><ChevronDown size={16} /></div></label><button className="process-button" disabled={loading || !file} onClick={processMeeting}>{loading ? <><span className="spinner" /> Processing...</> : <><Play size={16} fill="currentColor" /> Summarize meeting</>}</button></div>{error && <p className="error sans">{error}</p>}</div>
